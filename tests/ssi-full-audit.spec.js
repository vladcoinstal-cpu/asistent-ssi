...
function hasSourceDataForField(rawText, fieldLabel, subpointCode = "") {
  ...
  if (/^3\.3/.test(sp)) {
    ...
    if (/marcarea cailor de evacuare/.test(labelNorm)) return /(indicatoare\s+de\s+evacuare|panouri\s+de\s+semnalizare\s+pentru\s+evacuare|iluminat\s+de\s+siguranta\s+pentru\s+evacuare)/i.test(srcNorm);
  ...
  if (/căilor de evacuare|cailor de evacuare/.test(label)) {
    if (/marcarea/.test(labelNorm)) return /(indicatoare\s+de\s+evacuare|panouri\s+de\s+semnalizare\s+pentru\s+evacuare|iluminat\s+de\s+siguranta\s+pentru\s+evacuare)/i.test(srcNorm);
